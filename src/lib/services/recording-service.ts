...
import { recordings, recordingTypeEnum } from '../db/schema';
...
export const recordingService = {
  ...
  async deleteRecording(id: string) {
    const [recording] = await db.select({ fileUrl: recordings.fileUrl }).from(recordings).where(eq(recordings.id, id));
